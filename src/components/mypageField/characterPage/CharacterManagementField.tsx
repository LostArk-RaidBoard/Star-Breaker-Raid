...
import NewCharacterManager from '@/components/mypageField/characterPage/NewCharacterManager'
import CharacterDatabaseField from '@/components/mypageField/characterPage/CharacterDatabaseField'
import CharacterClassImageProvider from '@/components/utils/CharacterClassImageProvider'
...
export default function CharacterManagementField({ dbCharacter, userId }: Props) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (const item of data) {
          CharacterClassImageProvider(item)
        }
  ...
  const handlerOneCharacter = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        CharacterClassImageProvider(characterList)
    ...
    <>
      {userId ? (
        <div className='flex h-full w-full flex-col p-4'>
              ...
              <div className='flex flex-col'>
                ...
                >
                  <InputLayout
                    ...
                    setCSS={'rounded-md h-12 w-full sm:grow p-3 text-base'}
                    setValue={setMainCharacter}
                    value={mainCharacter}
                  />

                  <button className='flex-none rounded-md border bg-gray-800 px-4 py-3 text-gray-100 hover:bg-gray-500'>
                    확인
                  </button>
                </form>
          ...
          {/* 데이터베이스에서 보이는 장소 */}
          <CharacterDatabaseField userId={userId} dbCharacter={dbCharacter} />
        </div>
      ) : (
