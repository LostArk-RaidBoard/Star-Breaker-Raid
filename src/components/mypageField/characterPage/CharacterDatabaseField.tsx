 ...
 * @returns
 */
export default function CharacterDatabaseField({ userId, dbCharacter }: Props) {
    ...
    for (const item of characterList) {
      try {
        ...
        if (response.ok) {
          ...
          resultList.push(await CharacterSaveService(characterList, userId)) // 함수로 호출
          submit()
        }
      } catch (error) {
  ...
  return (
    <div className='mt-4 flex flex-col'>
      <h2 className='text-lg font-extrabold text-gray-900'>캐릭터</h2>
      ...
      <div className='mt-2 flex flex-col items-center justify-center'>
        ...
        <button
          className='mt-2 w-24 rounded-md border bg-gray-800 px-4 py-3 text-gray-100 hover:bg-gray-500'
